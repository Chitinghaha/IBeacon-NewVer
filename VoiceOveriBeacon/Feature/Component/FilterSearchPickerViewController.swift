...
class FilterSearchPickerViewController: UIViewController {
    ...
    func setupViews() {
        ...
        if (!self.viewModel.isCancelButtonHidden) {
            headerView.addSubviews(closeButton)
        }
        if (!self.viewModel.isSearchViewHidden) {
            headerView.addSubviews(searchBar)
        }
    ...
    // MARK: - init
    init(viewModel: FilterSearchPickerViewModel) {
...
//            .store(in: &cancellable)
//
//        viewModel.$isCancelButtonHidden
//            .receive(on: RunLoop.main)
//            .sink {
//                self.closeButton.isHidden = $0
//            }
//            .store(in: &cancellable)
    ...
    func makeDataSource() -> UITableViewDiffableDataSource<String, FSPCellViewModel> {
        ...
        return UITableViewDiffableDataSource(
            tableView: tableView) { tableView, indexPath, cellViewModel in
                ...
                cell.contentLabel.font = self.viewModel.config.contentTextFont
